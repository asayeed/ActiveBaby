...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_dir",
        default="./checkpoints/ACLM_elc_bert_base_half_24_50K_EP2",
        type=str,
    ...
    parser.add_argument(
        "--wandb_name", type=str, default="ACLM_ELC-BERT_dist", help="WANDB run name."
    )
    parser.add_argument(
        "--wandb_in", type=str, default="ACLM_ELC-BERT_dist_0", help="WANDB run in."
    )
...
def setup_training(args):
    ...
    # rank = torch.distributed.get_rank()  # Rank of the current process
    local_rank = rank % args.n_gpu  # Local rank within the node (single node)
...
def setup_training_single(args):
    rank = torch.cuda.device_count()
    seed_everything(args.seed + rank)
    gpus_per_node = torch.cuda.device_count()
...
def save(model, optimizer, grad_scaler, scheduler, global_step, epoch, args):
    checkpoint_path = f"{args.output_dir}/model_epoch{epoch}.bin"
    if is_main_process():
...
if __name__ == "__main__":
    ...
    if args.checkpoint_path is not None:
        ...
        args.wandb_id = (
            wandb.util.generate_id() if int(local_rank) == 0 else 0
        )
    ...
    # Load ACLM data
    train_data_df = pd.read_csv("data/processed24/cached_128.csv")
