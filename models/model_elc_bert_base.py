...
class Bert(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, masked_lm_labels=None):
        ...
        subword_prediction, masked_lm_labels = self.classifier(
            contextualized_embeddings, masked_lm_labels
        )
        return subword_prediction, masked_lm_labels
...
class MaskClassifier(nn.Module):
    ...
    def forward(self, x, masked_lm_labels=None):
        ...
        #         masked_lm_labels.flatten().squeeze(),
        #     )
        x = self.nonlinearity(x)
        return x, masked_lm_labels
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, relative_embedding):
        ...
        # print("===> shape attention_scores: [B, H, Tq, Tk]", attention_scores.size())
        ...
        # print("===> shape [B, H, Tq, Tr]", attention_scores_qp.size())

        attention_scores_pk = torch.einsum(
            "bhkd,qhd->bhqk", key * self.scale, query_pos
        )  # shape: [B, H, Tr, Tk]
        # print("===> shape [B, H, Tr, Tk]", attention_scores_pk.size())
        ...
        # print("===> shape [B, H, Tq, Tk]", attention_scores_qp.size())
        attention_scores_pk = attention_scores_pk.gather(
            dim=-2, index=position_indices
        )  # shape: [B, H, Tq, Tk]
        # print("===> shape [B, H, Tq, Tk]", attention_scores_pk.size())
        ...
        # shape: [B*H, Q, D]
        # print("===> shape [B*H, Q, D]", attention_probs.size())
        context = torch.bmm(attention_probs.flatten(0, 1), value)
        # shape: [Q, B, H*D]
        # print("===> shape [Q, B, H*D]", context.size())
